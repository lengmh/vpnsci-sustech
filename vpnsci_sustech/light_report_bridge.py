"""Light report bridge for rendering vpnsci search sessions with bundled assets."""
...
def _ensure_report_tool_scripts_path() -> None:
    from .report_tools import bundled_tool_root

    tool_root = bundled_tool_root()
    if str(tool_root) not in sys.path:
        sys.path.insert(0, str(tool_root))


def _disabled_discovery_curve(
    *,
    ...
    _ensure_report_tool_scripts_path()
    from scripts.discovery_curve import disabled_curve_payload
...
def _strict_discovery_curve(snapshots: list[dict], *, scope: str) -> dict:
    _ensure_report_tool_scripts_path()
    from scripts.discovery_curve import build_discovery_curve_payload

    return build_discovery_curve_payload(snapshots, scope=scope)
...
def _variant_marker(variant_type: str, query: str) -> str:
    return f"{variant_type or 'unknown'}:{query or ''}"


def _session_stage_markers(session: SearchSession) -> list[str]:
    markers: list[str] = []
    ...
    def add(variant_type: str, query: str) -> None:
        ...
        if not query:
            return
        marker = _variant_marker(variant_type or "unknown", query)
        if marker not in markers:
            markers.append(marker)

    filters = session.filters if isinstance(session.filters, dict) else {}
    for variant in filters.get("query_variants", []) or []:
        if isinstance(variant, dict):
            add(str(variant.get("variant_type") or variant.get("type") or "unknown"), str(variant.get("query") or ""))

    for variant in _query_variants_from_session(session):
        add(str(variant.get("type") or "unknown"), str(variant.get("query") or ""))

    return markers


def _hit_stage_markers(hit: SearchHit) -> list[str]:
    markers: list[str] = []
    ...
    if hit.query_variant:
        add(_variant_marker(hit.query_variant_type or "unknown", hit.query_variant))
    for marker in hit.query_variants:
        if ":" in marker:
            add(marker)
        else:
            add(_variant_marker("unknown", marker))
    return markers


def _build_seed_discovery_snapshots(session: SearchSession, papers: list[dict]) -> list[dict]:
    stage_markers = _session_stage_markers(session)
    if len(stage_markers) < 3:
        return []

    stage_index = {marker: index for index, marker in enumerate(stage_markers)}
    paper_stage_indexes: list[int] = []
    for hit in session.hits:
        indexes = [stage_index[marker] for marker in _hit_stage_markers(hit) if marker in stage_index]
        if not indexes:
            return []
        paper_stage_indexes.append(min(indexes))

    if len(paper_stage_indexes) != len(papers):
        return []
    ...
    for index, marker in enumerate(stage_markers):
        included = [
            paper
            for paper, paper_stage_index in zip(papers, paper_stage_indexes)
            if paper_stage_index <= index
        ]
        valid_rcs = [
            rcs
        ...
        snapshots.append(
            {
                "stage": marker,
                "papers_evaluated": len(included),
                "highly_relevant_count": sum(1 for rcs in valid_rcs if rcs >= 7),
                "valid_rcs_count": len(valid_rcs),
            }
        )
    return snapshots
...
def _build_chart_data(
    ...
    *,
    ...
    display_query: str = "",
    language: str = "",
    discovery_curve: dict | None = None,
) -> dict:
    ...
    discovery_curve = discovery_curve or _disabled_discovery_curve(
        status="insufficient_trace",
        reason="缺少可解释的执行轨迹，当前不输出覆盖率/饱和度结论。",
        scope="seed_set",
    )
...
def _write_materialized_data(
    ...
    *,
    ...
    seed_discovery_snapshots = _build_seed_discovery_snapshots(session, papers)
    has_valid_rcs = any(_rcs_value_if_valid(paper) is not None for paper in papers)
    if not has_valid_rcs:
        discovery_curve = _disabled_discovery_curve(
            status="no_valid_rcs",
            reason="未执行正式 RCS 分类，当前不输出覆盖率/饱和度结论。",
            scope="seed_set",
        )
    elif not seed_discovery_snapshots:
        discovery_curve = _disabled_discovery_curve(
            status="insufficient_trace",
            reason="缺少可归属到检索阶段的执行轨迹，当前不输出覆盖率/饱和度结论。",
            scope="seed_set",
        )
    else:
        discovery_curve = _strict_discovery_curve(seed_discovery_snapshots, scope="seed_set")
    ...
    if discovery_curve.get("mode") == "disabled":
        quality_profile = dict(quality_profile)
        quality_profile["discovery_curve_mode"] = "disabled"
    ...
    helper_disabled_discovery_curve = discovery_curve.get("mode") == "disabled"
    recovery_capability = str(origin.get("report_recovery_capability") or "")
    quality_should_override_discovery = quality_profile["discovery_curve_mode"] == "disabled" and (
        not helper_disabled_discovery_curve
        or "actual_queries" in missing_fields
        or (recovery_capability and recovery_capability != "standard")
    )
    if quality_should_override_discovery:
    ...
    elif helper_disabled_discovery_curve:
        discovery_curve["coverage_estimate"] = None
        discovery_curve["ci_low"] = None
        discovery_curve["ci_high"] = None
        discovery_curve["estimated_total_relevant"] = None
