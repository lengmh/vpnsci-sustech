"""Deterministic helpers for agent-owned theme label postprocess.

This module no longer calls any external provider directly. The default design
is:

1. Python builds a normalized request payload from `raw_theme_treemap`.
2. The current host Agent performs the conservative "manual" theme cleanup.
3. Python applies the supplied result and records trace metadata.

This keeps the execution surface agent-agnostic: Codex, Claude Code, or any
other capable agent can all supply the same postprocess result shape.
"""
...
def build_theme_postprocess_request(
    ...
    *,
    ...
    """Return `(request_payload, trace)` for an agent-owned postprocess step."""
...
def apply_theme_postprocess_result(
    ...
    *,
    ...
    """Apply one agent-supplied postprocess result onto a raw theme treemap."""
