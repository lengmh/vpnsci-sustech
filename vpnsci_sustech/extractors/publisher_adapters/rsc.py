"""HTML extractor for RSC (Royal Society of Chemistry) articles."""
...
def can_handle(url: str) -> bool:
    ...
    return "pubs.rsc.org" in url.lower()


def extract(html: str, url: str = "") -> dict:
    """Extract paper content from RSC Publications HTML."""
...
def _extract_authors(soup: BeautifulSoup) -> list[str]:
    ...
    for meta in soup.select("meta[name='citation_author']"):
        ...
        if name:
            authors.append(name)
    if not authors:
        for el in soup.select(".article__authors a, .authors-list a"):
...
def _extract_body(soup: BeautifulSoup) -> str:
    ...
    body = soup.select_one("div#article-body") or soup.select_one("div.article__body")
    if body:
        for section in body.find_all(["section", "div"], recursive=False):
            ...
            if heading_text.lower() in ("abstract", "references", "notes and references"):
                continue
...
def _extract_references(soup: BeautifulSoup) -> list[str]:
    refs = []
    for selector in ["div.references", "section.references", "#references"]:
